...
static int init_model(const char *model_path, rknn_app_context_t *app_ctx,int core_num)
{
  ...
  qDebug()<<"----------------------------------core_mask:"<<(int)core_mask<<"   "<<core_mask;
...
static int inference_model(rknn_app_context_t *app_ctx, image_frame_t *img, detect_result_group_t *detect_result)
{
  ...
  qDebug()<<"ctx     "<<ctx<<"  "<<img->width<<"  "<<img->height<<"  "<<img->width_stride<<"   "<<img->height_stride<<"    "<<img->format;
  ...
  qDebug() << "resize_buf  -----------------------                      :"<<(uchar*)resize_buf;
...
void mpp_decoder_frame_callback(void *userdata, int width_stride, int height_stride, int width, int height, int format, int fd, void *data ,void * pUser,ShowImageCallBack showimagecallback)
{
  ...
  if (ret != 0)
  {
    ...
    qDebug()<<"inference model error   ctx"<<ctx;
    //goto RET;
  ...
  // Draw objects
  printf("detect_result.count:%d\n",detect_result.count);
  //qDebug()<<"detect_result.count   "<<detect_result.count;
  for (int i = 0; i < detect_result.count; i++)
  {
    ...
    draw_rectangle_yuv420sp((unsigned char *)mpp_frame_addr, width_stride, height_stride, x1, y1, x2 - x1 + 1, y2 - y1 + 1, 0x00FF0000, 1);
  }

  //yuv420sp  to rgb888
  char *dst_buff = (char *)malloc(width_stride*height_stride*get_bpp_from_format(DST_FORMAT));
  char *b = (char *)mpp_frame_addr;
  ret = 0;
  //相当于初始化dst_buff
  memset(dst_buff,0x00,width_stride*height_stride*get_bpp_from_format(DST_FORMAT));

  rga_buffer_t src1 = wrapbuffer_virtualaddr(b, width_stride, height_stride, SRC_FORMAT);
  rga_buffer_t dst1 = wrapbuffer_virtualaddr(dst_buff, width_stride, height_stride, DST_FORMAT);
  if(src1.width == 0 || dst1.width == 0) {
      qDebug() << "wrapbuffer_virtualaddr error!";
  }

  src1.format = SRC_FORMAT;
  dst1.format = DST_FORMAT;
...
//  }
  ...
  int STATUS = 0;
  STATUS = imcvtcolor(src1, dst1, src1.format, dst1.format, IM_COLOR_SPACE_DEFAULT, 1);
...
//    QByteArray imageData((const char*)enc_data,enc_data_size);
//    QImage img;
...
//        img.save("output.jpg","JPEG",80);
  ...
  //qDebug() << "enc_buf_size :"<<enc_buf_size;
  //qDebug() << "enc_data_size :"<<enc_data_size;
  fwrite(enc_data, 1, enc_data_size, ctx->out_fp);
#endif

  if(showimagecallback /* && IM_STATUS_SUCCESS == STATUS*/ )
  {
//      uchar *data_vir =(uchar *)mpp_buffer_get_ptr(mpp_frame);
//      showimagecallback(0,data_vir,pUser,width_stride,height_stride);

//    showimagecallback(0,(uchar*)enc_data,pUser,width_stride,height_stride);

    showimagecallback(0,(uchar*)dst_buff,pUser,width_stride,height_stride);
...
//void MppRknnVideo::setCallBack(ShowImageCallBack _pSHowImage,void* pUser)
//{
//  m_pShowImage_CallBack = _pSHowImage;
//}

MppRknnVideo::MppRknnVideo(ShowImageCallBack _call_Back_ShowInQt,void*_user,QString rtsp_dddr,int videotype,int core_num)
{
...
//  char *video_name = argv[2];
  ...
  QString modelname = "/home/topeet/rknpu2/examples/rknn_yolov5_demo/model/RK3588/yolov5s-640-640_rk3588.rknn";
  QByteArray byteArray = modelname.toUtf8();
  char *model_name = byteArray.data();

  //QString videoname = "rtsp://192.168.0.177:8554/stream";
  //QString videoname = "rtsp://admin:wetech6666@124.71.112.94:20023";
  //QString videoname = "rtsp://admin:QSDBNT@192.168.0.136:554/h264/ch1/main/av_stream";
  //QByteArray byteArray_videoname = videoname.toUtf8();
  QByteArray byteArray_videoname = rtsp_dddr.toUtf8();
  char *video_name = byteArray_videoname.data();

  int video_type = videotype;

  rknn_app_context_t app_ctx;
  memset(&app_ctx, 0, sizeof(rknn_app_context_t));

  ret = init_model(model_name, &app_ctx,core_num);
  if (ret != 0)
  {
    printf("init model fail\n");
    qDebug()<<"1";
  }
qDebug()<<"MppRknnVideo thread in run is  "<<QThread::currentThread();
  if (app_ctx.decoder == NULL)
  {
    MppDecoder *decoder = new MppDecoder();
    decoder->Init(video_type, 30, &app_ctx);
//    this->setCallBack(ImageCallBackInner);
    decoder->SetCallback(mpp_decoder_frame_callback);
    decoder->SetCallback_ToQt(_call_Back_ShowInQt,_user);
