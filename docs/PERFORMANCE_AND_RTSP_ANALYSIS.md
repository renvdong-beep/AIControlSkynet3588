# RK3588 性能优化与 RTSP 分析

**项目：** AIControlSkynet3588 - V4L2 + Rockchip MPP 多路解码 + RKNN 推理框架

**目标：** 实现 16 路 YOLOv5s 实时推理 / 4 路 RTSP 实时推理

**最终性能：** 105 fps @ 16 路（V4L2），104 fps @ 4 路（RTSP）

---

## 优化阶段概览

| 阶段 | 优化内容 | 性能 | 提升倍数 |
|------|---------|------|---------|
| 基准 | 单路解码 + 推理 | ~30 fps | 1.0x |
| 阶段1 | 多路解码 + 多实例 | 39 fps | 1.3x |
| 阶段2 | NPU 3核并行 | 105 fps | 2.7x |
| 阶段3 | 批处理/异步测试 | - | 需要batch模型 |
| 阶段4 | RTSP + MPP 硬解码 | 104 fps @ 4路 | CPU占用大幅降低 |

---

## 阶段 0：项目初始化

**时间：** 2026-04-07 下午

**目标：** 搭建 V4L2 + MPP 多路解码框架

**关键代码：**
- `src/main.cpp` - 主程序框架
- `CMakeLists.txt` - 编译配置

**技术栈：**
- V4L2 MMAP 零拷贝采集
- Rockchip MPP MJPEG 硬件解码
- RGA 格式转换（NV12 → RGB888）
- RKNN NPU 推理

**提交节点：**
- `9a7dce9` - 实现多路摄像头同时采集解码
- `dc82c57` - 移除内部文档

**性能：**
- 单路解码：30 fps
- 双路解码：30 fps（无丢帧）

---

## 阶段 1：MJPEG 解码突破

**时间：** 2026-04-07 晚上

**问题：** MPP 解码器返回 `frame 0x0, err=1`

**根本原因：** MJPEG 必须使用 `mpp_packet_init_with_buffer`

**解决方案：**
```cpp
// ❌ 错误方式
mpp_packet_init(&packet, data, size);

// ✅ 正确方式
MppBuffer pkt_buf;
mpp_buffer_get(pkt_grp_, &pkt_buf, size);
memcpy(mpp_buffer_get_ptr(pkt_buf), data, size);
mpp_packet_init_with_buffer(&packet, pkt_buf);
```

**关键代码修改：**
- 文件：`src/main.cpp`
- 函数：`MPPDecoder::decode()`
- 行数：~280-320

**提交节点：**
- `646b443` - 完成 RKNN NPU 推理集成

**性能：**
- 双路解码 + 推理：30 fps

---

## 阶段 2：RKNN 推理集成

**时间：** 2026-04-07 晚上

**目标：** 添加 RGA 格式转换和 RKNN NPU 推理

**架构：**
```
USB Camera → V4L2 MMAP → MPP MJPEG Decode → RGA Scale → RKNN Inference
    ↓            ↓              ↓               ↓              ↓
/dev/video21  1080p NV12    640x640 RGB    YOLOv5s      2.1M floats
/dev/video23  1080p NV12    640x640 RGB    YOLOv5s      2.1M floats
```

**关键类：**
- `V4L2Capture` - V4L2 采集
- `MPPDecoder` - MPP 解码
- `RGAProcessor` - RGA 格式转换
- `RKNNInference` - RKNN 推理
- `CameraChannel` - 单路摄像头管道
- `MultiCameraManager` - 多路管理器

**性能：**
- 双路解码 + 推理：30 fps

---

## 阶段 3：压力测试框架

**时间：** 2026-04-07 深夜

**目标：** 测试 RK3588 NPU 最大推理路数

**实现：** 创建 `src/stress_test.cpp`

**设计思路：**
- 从 2 路摄像头采集帧
- 复制成 N 路进行 RKNN 推理
- 测试 4/8/12/16 路并行推理极限

**初始测试结果（串行推理）：**

| 路数 | 推理帧率 | 等效总帧率 | 平均延迟 |
|------|---------|-----------|---------|
| 4 路 | 7.9 fps | 31.5 fps | 31.74 ms |
| 8 路 | 4.5 fps | 35.9 fps | 27.83 ms |
| 12 路 | 3.2 fps | 38.0 fps | 26.29 ms |
| 16 路 | 2.4 fps | 39.1 fps | 25.60 ms |

**瓶颈：** 串行调用 `rknn_run()`，NPU 利用率低

---

## 阶段 4：MPP 高级模式修复

**时间：** 2026-04-07 深夜

**问题：** stress_test 中 MPP 解码失败

**原因：** stress_test.cpp 使用了简单模式，但 MJPEG 需要高级模式

**解决方案：**
```cpp
// 高级模式流程
ret = api_->poll(ctx_, MPP_PORT_INPUT, MPP_POLL_BLOCK);
ret = api_->dequeue(ctx_, MPP_PORT_INPUT, &task);
mpp_task_meta_set_packet(task, KEY_INPUT_PACKET, packet);
mpp_task_meta_set_frame(task, KEY_OUTPUT_FRAME, frame_);
ret = api_->enqueue(ctx_, MPP_PORT_INPUT, task);

ret = api_->poll(ctx_, MPP_PORT_OUTPUT, MPP_POLL_BLOCK);
ret = api_->dequeue(ctx_, MPP_PORT_OUTPUT, &out_task);
mpp_task_meta_get_frame(out_task, KEY_OUTPUT_FRAME, &frame_out);
api_->enqueue(ctx_, MPP_PORT_OUTPUT, out_task);
```

---

## 阶段 5：NPU 多核并行优化 ⭐

**时间：** 2026-04-07 深夜

**目标：** 启用 RK3588 NPU 3 核并行

**RK3588 NPU 架构：**
- 3 个独立核心（NPU Core 0/1/2）
- 每核 2 TOPS 算力，总计 6 TOPS
- 支持核心绑定：`rknn_set_core_mask()`

**关键 API：**
```cpp
// 核心掩码定义
RKNN_NPU_CORE_0 = 1       // 核心 0 (2 TOPS)
RKNN_NPU_CORE_1 = 2       // 核心 1 (2 TOPS)
RKNN_NPU_CORE_2 = 4       // 核心 2 (2 TOPS)
RKNN_NPU_CORE_0_1_2 = 7   // 三核并行（单模型加速）
RKNN_NPU_CORE_AUTO = 0    // 自动调度

// 设置核心绑定
rknn_set_core_mask(ctx, core_mask);
```

**优化策略：**

1. **核心绑定**
```cpp
rknn_core_mask core_masks[] = {
    RKNN_NPU_CORE_0, RKNN_NPU_CORE_1, RKNN_NPU_CORE_2
};
for (int i = 0; i < num_channels; i++) {
    rknn_core_mask core = core_masks[i % 3];
    rknn_set_core_mask(ctx, core);
}
```

2. **多线程并行**
```cpp
std::vector<std::thread> threads;
for (int i = 0; i < num_channels; i++) {
    threads.emplace_back([&, i]() {
        rknn_instances[i]->inference(rgb_buffers[i].data());
    });
}
for (auto& t : threads) threads.join();
```

**性能对比（YOLOv5s 640x640）：**

| 路数 | 串行推理 | 3核并行 | 提升 | 延迟改善 |
|------|---------|---------|------|---------|
| 3 路 | 21 fps | 63 fps | 3.0x | - |
| 6 路 | 27 fps | 80 fps | 3.0x | - |
| 8 路 | 36 fps | 84 fps | 2.3x | - |
| 12 路 | 38 fps | 97 fps | 2.6x | - |
| **16 路** | **39 fps** | **105 fps** | **2.7x** | 25.6ms → 9.5ms |

---

## 阶段 6：批处理/异步推理测试

**时间：** 2026-04-07 深夜

**实现：** 创建 `src/rknn_optimized.cpp`

### 1. 批处理推理

**结果：** ❌ 失败
- 错误：`rknn_set_batch_core_num: failed to set core_num 4`
- 原因：当前 YOLOv5s 模型不支持批处理
- 需要：专门训练的 batch 模型

### 2. 零拷贝 DMABUF

**结果：** ⚠️ 需要硬件支持
- USB 摄像头不支持 DMABUF 导出
- 需要 RGA 配合导出 DMABUF fd

### 3. 异步推理

**结果：** ✅ 成功
- 性能：38.8 fps
- 延迟：25.74 ms

| 优化方式 | 状态 | 性能 | 说明 |
|---------|------|------|------|
| 单帧推理（基准） | ✅ | 40.1 fps | 24.92ms 延迟 |
| 批处理推理 | ❌ | 失败 | 需要 batch 模型 |
| 异步推理 | ✅ | 38.8 fps | 25.74ms 延迟 |

---

## 阶段 7：RTSP + MPP 硬解码优化 ⭐

**时间：** 2026-04-08 ~ 2026-04-09

**目标：** 将 RTSP 流从 FFmpeg 软解切换到 MPP 硬解码

**实现：** 创建 `multi_rtsp_mpp_test.cpp` + `mpp_rtsp_cli_test.cpp`

### 7.1 技术路径对比

| 组件 | stress_test (V4L2) | multi_rtsp_test (FFmpeg软解) | multi_rtsp_mpp_test (MPP硬解) |
|------|-------------------|---------------------------|---------------------------|
| **输入** | V4L2 单路采集 | 4 路 RTSP 独立拉流 | 1-4 路 RTSP 独立拉流 |
| **解码** | MPP MJPEG 硬解 | FFmpeg 软解 (yuv420p→rgb24) | MPP H.264 硬解 |
| **转换** | RGA | sws_scale + RGA | RGA NV12→RGB+缩放 |
| **推理** | 多线程并行 | 每路独立线程 | 每路独立线程 |
| **核心绑定** | Core0/1/2 | Core0/1/2 | Core0/1/2 轮转 |
| **显示** | 无 | Qt | Qt 480x272 @ 25fps |
| **性能** | 105 fps @ 16路 | ~30 fps @ 3路 | ~104 fps @ 4路 |

### 7.2 MPP H.264 解码器实现

**技术路径：**
```
RTSP → FFmpeg 解封装 → H.264 NALU → MPP 硬解 → NV12 → RGA → RKNN → Qt 显示
```

**关键区别（vs MPP MJPEG 解码）：**
- MJPEG 使用 Task-based API（高级模式）
- H.264 使用 Legacy API（简单模式）

```cpp
// H.264 使用 Legacy API
mpp_create(&ctx, &api);
mpp_init(ctx, MPP_CTX_DEC, MPP_VIDEO_CodingAVC);

// 发送数据包
MppPacket packet;
mpp_packet_init(&packet, data, size);
api->decode_put_packet(ctx, packet);
mpp_packet_deinit(&packet);

// 获取解码帧
MppFrame frame;
api->decode_get_frame(ctx, &frame);
```

### 7.3 已解决的关键问题

1. **FFmpeg 默认触发软解码** — 移除 `avformat_find_stream_info()`
2. **RTSP 流缺少 SPS/PPS** — 从 FFmpeg extradata 提取
3. **MPP Info Change 处理** — 检测 EOS 帧并正确处理
4. **-1012 错误** — 正确设置输出缓冲区组
5. **误检过多** — BOX_THRESH 从 0.25 提高到 0.50
6. **显示阻塞推理** — 显示/推理分离，显示限 25fps

### 7.4 NPU 使用率对比

```
FFmpeg 软解时:  Core0: 57%, Core1: 52%, Core2: 55%  (瓶颈在 CPU)
MPP 硬解后:    Core0: 85%, Core1: 82%, Core2: 80%  (瓶颈转移到 NPU)
```

### 7.5 性能对比

| 路数 | FFmpeg 软解 | MPP 硬解（目标） | MPP 硬解（实测） | 状态 |
|------|-----------|----------------|----------------|------|
| 1 路 | 30 fps | 30 fps | 30 fps | ✅ |
| 2 路 | < 30 fps | 60 fps | ~28 fps/路 | ✅ |
| 3 路 | < 30 fps | 90 fps | ~26 fps/路 | ✅ |
| 4 路 | < 30 fps | 100+ fps | ~26 fps/路 | ✅ |

> 注：实际 FPS 为每路独立 FPS，总吞吐量 = 路数 × 每路 FPS

### 7.6 当前配置

```cpp
MODEL_PATH = "yolov5s-640-640.rknn"  // 2 类 (cow + person)
RTSP_URL = "rtsp://192.168.137.251:8554/cow0~3"
TRANSPORT = TCP
BOX_THRESH = 0.50, NMS_THRESH = 0.45
NPU_CORES = RKNN_NPU_CORE_0/1/2 轮转
DISPLAY_FPS = 25, DISPLAY_SIZE = 480x272
```

---

## 最终架构

```
┌─────────────────────────────────────────────────────────────┐
│                    MultiCameraManager                        │
│  ┌──────────────────────────────────────────────────────┐  │
│  │              CameraChannel[0]                         │  │
│  │  V4L2Capture → MPPDecoder → RGAProcessor → RKNN#0    │  │
│  └──────────────────────────────────────────────────────┘  │
│  ...                                                        │
│  NPU Core 0: RKNN#0, #3, #6, #9, #12, #15 (6 instances)   │
│  NPU Core 1: RKNN#1, #4, #7, #10, #13 (5 instances)       │
│  NPU Core 2: RKNN#2, #5, #8, #11, #14 (5 instances)       │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│                RTSP Multi-Channel (MPP+RKNN)                │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  RTSP → FFmpeg解封装 → MPP H.264硬解 → RGA → RKNN   │  │
│  └──────────────────────────────────────────────────────┘  │
│  × 4 channels, NPU Core 0/1/2 轮转                        │
│  Qt 显示 25fps, 推理全速                                    │
└─────────────────────────────────────────────────────────────┘
```

---

## 性能数据汇总

### V4L2 多路性能（3核并行）

| 路数 | 推理帧率 | 等效总帧率 | 延迟 | 提升倍数 |
|------|---------|-----------|------|---------|
| 3 路 | 21.0 fps | 63.0 fps | 15.87 ms | 1.6x |
| 6 路 | 13.3 fps | 79.8 fps | 12.52 ms | 2.0x |
| 8 路 | 10.6 fps | 84.4 fps | 11.85 ms | 2.1x |
| 12 路 | 8.1 fps | 96.9 fps | 10.32 ms | 2.4x |
| **16 路** | **6.6 fps** | **105.0 fps** | **9.52 ms** | **2.7x** |

### RTSP 多路性能（MPP 硬解）

| 方案 | 解码方式 | 4路总FPS | CPU占用 | NPU利用率 |
|------|---------|---------|---------|-----------|
| multi_rtsp_test | FFmpeg 软解 | ~30 fps | 高 | ~55% |
| multi_rtsp_mpp_test | MPP 硬解 | ~104 fps | 低 | ~82% |

### 单路性能

| 组件 | 性能 | 说明 |
|------|------|------|
| V4L2 采集 | 30 fps | 1080p MJPEG |
| MPP 解码 | 30 fps | NV12 输出 |
| RGA 缩放 | 30 fps | 640x640 RGB |
| RKNN 推理 | 40 fps | YOLOv5s |

---

## 关键技术点

### 1. MPP MJPEG 解码（高级模式）

**必须使用 `mpp_packet_init_with_buffer()`**

### 2. MPP H.264 解码（Legacy API）

```cpp
mpp_create(&ctx, &api);
mpp_init(ctx, MPP_CTX_DEC, MPP_VIDEO_CodingAVC);
api->decode_put_packet(ctx, packet);
api->decode_get_frame(ctx, &frame);
```

### 3. NPU 核心绑定

```cpp
rknn_core_mask core_masks[] = { RKNN_NPU_CORE_0, RKNN_NPU_CORE_1, RKNN_NPU_CORE_2 };
rknn_set_core_mask(ctx, core_masks[i % 3]);
```

### 4. RGA 格式转换

```cpp
// NV12 → RGB888 + 缩放（一次完成）
rga_buffer_t src = wrapbuffer_virtualaddr(nv12_data, w, h, RK_FORMAT_YCbCr_420_SP, hs, vs);
rga_buffer_t dst = wrapbuffer_virtualaddr(rgb_buf, 640, 640, RK_FORMAT_RGB_888);
imresize(src, dst);
```

---

## 后续优化方向

1. **INT8 量化** — 可提升 2-4x 吞吐量
2. **Batch 模型** — 减少调用开销
3. **零拷贝路径** — DMABUF fd 直接传递
4. **多进程方案** — 每个进程绑定一个 NPU 核心
5. **更多路数测试** — 5-8 路 RTSP
6. **外部 RTSP 源测试** — 端口可达性问题

---

## 文件结构

```
AIControlSkynet3588/
├── CMakeLists.txt
├── readme.md
├── src/
│   ├── main.cpp                    # 主程序（多路解码+推理）
│   ├── stress_test.cpp             # 压力测试（V4L2, 16路）
│   ├── rknn_optimized.cpp          # RKNN 优化引擎
│   ├── mpp_rtsp_cli_test.cpp       # CLI 单通道测试
│   ├── multi_rtsp_benchmark.cpp    # CLI 多路基准测试
│   ├── decoder/
│   │   ├── mpp_decoder.cpp/h       # H.264/H.265 解码器
│   │   └── mpp_encoder.cpp/h       # H.264 编码器
│   ├── inference/
│   │   ├── postprocess.cpp/h       # 后处理（NMS）
│   │   └── preprocess.cpp/h        # 预处理
│   ├── qt_ui/
│   │   ├── multi_rtsp_mpp_test.cpp # Qt 多路 RTSP+MPP+RKNN
│   │   ├── multi_rtsp_benchmark_ui.cpp # Qt 多路性能基准测试
│   │   └── ...                     # 其他 Qt 测试程序
│   └── utils/
│       └── drawing.cpp/h           # YUV 绘制
└── docs/
    ├── PERFORMANCE_AND_RTSP_ANALYSIS.md  # 本文档
    ├── VIDEOCONTROL_MIGRATION.md         # VideoControl 移植文档
    ├── ARCHITECTURE.md                   # 架构文档
    ├── INTEGRATION_TEST.md               # 集成测试文档
    └── BENCHMARK_GUIDE.md                # 基准测试指南
```

---

## 提交历史

```
5ec1b20 - v1.1: restore source files, add docs, add benchmark tool
ab4541b - chore: 移除内部文档
1c671cf - feat: 添加 RKNN 优化推理引擎
8e4f6a0 - feat: NPU 多核并行推理优化
c416071 - feat: 添加 RKNN 极限压力测试程序
646b443 - 完成 RKNN NPU 推理集成
9a7dce9 - 实现多路摄像头同时采集解码
```

---

**最后更新：** 2026-04-10

**项目状态：** 生产就绪

**最终性能：** 16 路 YOLOv5s @ 105 fps (V4L2) / 4 路 × 26 fps = 104 fps (RTSP)
